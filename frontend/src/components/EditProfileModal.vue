  ...
  <div class="modal fixed inset-0 flex items-center justify-center">
    ...
    <div class="modal-content bg-white rounded-lg overflow-hidden shadow-md">
      <div class="px-6 py-4">
        ...
        <div class="mb-4">
          <img :src="newAvatarSrc" alt="Avatar" class="rounded rounded-full object-cover object-center p-2 bg-gray-200 w-24 h-24 mx-auto border border-gray-500" />
          <label class="block font-bold text-gray-700 mb-2" for="new-avatar">
            New Avatar Image
          </label>
          <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="new-avatar" type="file" @change="handleAvatarChange">
        </div>
...
export default {
  ...
  methods: {
    _handleAvatarChange(e) {
      ...
      this.newAvatarSrc = URL.createObjectURL(this.newAvatar);
    ...
    handleAvatarChange(event) {
      ...
      reader.onload = (event) => {
        ...
        img.onload = () => {
          ...
          canvas.toBlob((blob) => {
            this.newAvatar = new File([blob], file.name, { type: file.type });
            this.newAvatarSrc = URL.createObjectURL(this.newAvatar);
          }, file.type, 1);
        };
